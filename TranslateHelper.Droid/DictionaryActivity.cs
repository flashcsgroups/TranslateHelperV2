	...
	public class DictionaryActivity : Activity
	{
		...
		protected override void OnCreate (Bundle bundle)
		{
			...
			resultListView.ItemClick += (object sender, AdapterView.ItemClickEventArgs e) => {
				...
				var favStatePic = e.View.FindViewById<ImageView> (Resource.Id.FavoritesStatePic);
		...
		private async void AddToFavorites (string sourceText, TranslateResult result)
		{
			if (result.FavoritesId == 0) {
				...
				Android.Widget.Toast.MakeText (this, "Элемент уже в избранном!", Android.Widget.ToastLength.Short).Show ();
			}
		...
		void ShowScreenResolution ()
		{
			...
			TextView splashTextView = FindViewById<TextView> (Resource.Id.splashTextView);
			string px = "px:" + metrics.WidthPixels.ToString () + "x" + metrics.HeightPixels.ToString ();
			string dp = "dp:" + widthInDp.ToString () + "x" + heightInDp.ToString ();
			splashTextView.Text = "Готов что-нибудь перевести!" + System.Environment.NewLine + px + System.Environment.NewLine + dp;
		}
	}
}
